function [Test, Train, After] = MotorAdaptation_v4(n_trials,input_device,test_subject,folder_path)
%% 
...
% Define properties of the basic objects to draw
dotColor = [1 0 0]; % red dot as cursor
dotColor_rot = [0 255 0]; % green dot as rotated cursor
dotColor_target = [0 0 0]; % black dot as target
rectColor = [0 0 0];
...
nTriala = 1;
TrialDXY = [];
ttr = possiblePositions(randi(8,1)); % random positions (1 to 8)
...
while ~exitDemo
    ...
    if menu == 0
        ...
        while ~KbCheck
            ...
            line2 = '\n\n "slice straight" through a target (black dot)';
            line6 = '\n\n\n >> The red dot represents your position in the arena';
            line8 = '\n\n\n >> A smaller dot of the same color will appear giving you feedback on your performance';
    ...
    elseif menu == 1
        ...
        while nTrialt < NumberTrialsTrain+1
            ...
            if State == 1
                ...
                Screen('DrawDots', window, [r_rot(1)+xCenter r_rot(2)+yCenter], dotSizePix, dotColor_rot, [], 2);
                Screen('DrawDots', window, [r(1)+xCenter r(2)+yCenter], dotSizePix, dotColor, [], 2);
                
                % If cursor is out of bounds
                if rr > 0.45 * screenYpixels
                    Screen('DrawDots', window, [t1Xpos t1Ypos], dotSizePix, dotColor, [], 2);
%                     Screen('DrawDots', window, dotPositionMatrixTicks, dotSizesTicks, dotColorsMatrixTicks, dotCenterTicks, 2);
                    ...
                    Screen('DrawDots', window, dotPositionMatrixTicks, dotSizesTicks, dotColorsMatrixTicks, dotCenterTicks, 2);
                    Screen('DrawDots', window, [t1Xpos t1Ypos], dotSizePix, dotColor_target, [], 2);
                    Screen('DrawDots', window, dotPositionMatrix, dotSizes, dotColor, dotCenter, 2);
                    Screen('Flip', window);
    ...
    elseif menu == 3
        ...
        while nTrialp < NumberTrialsProtocol+1
            ...
            if State == 1
                ...
                % If cursor is out of bounds
                if rr > 0.45 * screenYpixels
                    Screen('DrawDots', window, [t1Xpos t1Ypos], dotSizePix, dotColor_rot, [], 2);
%                     Screen('DrawDots', window, dotPositionMatrixTicks, dotSizesTicks, dotColorsMatrixTicks, dotCenterTicks, 2);
                    Screen('Flip', window);
                    % Compute ending angle
                    theta = atan(r_rot(1)/r_rot(2));
                    if r_rot(2) < 0
                        t = theta+pi;
                    else
                        t = theta;
                    ...
                    Screen('DrawDots', window, dotPositionMatrixTicks, dotSizesTicks, dotColorsMatrixTicks, dotCenterTicks, 2);
                    Screen('DrawDots', window, [t1Xpos t1Ypos], dotSizePix, dotColor_target, [], 2);
                    Screen('DrawDots', window, dotPositionMatrix, dotSizes, dotColor_rot, dotCenter, 2);
                    Screen('Flip', window);
                    % Force some waiting time before next trial
                    WaitSecs(0.5);
                end
...
%                 dotColor = [1 0 1];
    ...
    elseif menu == 4
        ...
        while ~KbCheck
            ...
            line3 = '\n\n This is exactly the same as the first part of the experiment';
            line4 = '\n\n Press any key to continue';
            Screen('TextSize', window, 20 );
